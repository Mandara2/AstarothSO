...
class LoginWindow(QWidget):
    def __init__(self):
        ...
        self.perfiles = self.cargar_perfiles("InfoUsers/perfiles.json")
    ...
    def cargar_perfiles(self, file_path):
      try:
        with open(file_path, "r") as file:
          return json.load(file)
      except (FileNotFoundError, json.JSONDecodeError):
        print("Error al cargar el archivo JSON, usando perfiles predeterminados.")
        return {}
    ...
    def create_profile_buttons(self, parent_layout):
      # Crear el diseño de los botones de perfil
      ...
      for perfil, datos in self.perfiles.items():
        ...
        # Manejar errores si el ícono no está definido o no se encuentra
        icon_path = datos.get('icon', 'imagenes/iconos/usuarios/usuario_default.png')
        if not os.path.exists(icon_path):
          print(f"Icono no encontrado para el perfil '{perfil}'. Usando el ícono predeterminado.")
          icon_path = 'imagenes/iconos/usuarios/usuario_default.png'

        # Cargar y escalar la imagen del ícono
        pixmap = QPixmap(icon_path).scaled(
          100, 100, Qt.KeepAspectRatioByExpanding, Qt.SmoothTransformation
        )

        # Crear una máscara circular para el botón
        mask = QRegion(QRect(0, 0, 100, 100), QRegion.Ellipse)
        button.setMask(mask)

        # Aplicar la imagen como ícono al botón
        ...
        # Etiqueta con el nombre del perfil
        label = QLabel(perfil)
        label.setAlignment(Qt.AlignCenter)
        label.setStyleSheet("color: white; font-size: 16px;")

        # Agregar el botón y la etiqueta al widget del perfil
        profile_widget.addWidget(button)
        profile_widget.addWidget(label)
        profiles_layout.addLayout(profile_widget)

        # Guardar botón en el diccionario para referencia futura
    ...
    def select_profile(self, perfil):
      ...
      # Actualizar la selección de los botones
      ...
      # Obtener los datos del perfil seleccionado
      perfil_datos = self.perfiles.get(perfil, {})

      # Manejar si no existe un ícono en los datos del perfil o si la ruta no es válida
      icon_path = perfil_datos.get('icon', 'imagenes/iconos/usuarios/usuario_default.png')
      if not os.path.exists(icon_path):
        print(f"Icono no encontrado para el perfil '{perfil}'. Usando el ícono predeterminado.")
        icon_path = 'imagenes/iconos/usuarios/usuario_default.png'

      # Actualizar la etiqueta del nombre de usuario
      self.username_label.setText(perfil)

      # Actualizar el ícono del perfil seleccionado
      pixmap_icon = QPixmap(icon_path).scaled(100, 100, Qt.KeepAspectRatioByExpanding, Qt.SmoothTransformation)
      self.profile_icon.setPixmap(pixmap_icon)

      # Habilitar entrada de contraseña y botón de login
    ...
    def check_login(self):
      ...
      # Verificar los datos del perfil seleccionado
      perfil_datos = self.perfiles.get(self.selected_profile)

      if perfil_datos is None:
        QMessageBox.critical(self, 'Error', f'El perfil "{self.selected_profile}" no está definido.')
        return

      # Validar la contraseña
      if perfil_datos.get('password') == contrasena:
    ...
    def sleep_system(self):
      """Simula la suspensión del script de Python"""
      print("El sistema (script) está en suspensión por 5 segundos...")
      time.sleep(5)  # Suspender el script por 5 segundos
      print("El sistema (script) ha vuelto a la actividad.")

    def restart_system(self):
      """Reinicia el script de Python"""
      print("Reiniciando el sistema...")
      time.sleep(1)  # Tiempo para mostrar el mensaje antes de reiniciar
